...
namespace DashboardApp.Models
{
    ...
    public class Dashboard : DbConnection
    {
        ...
        public decimal RevenueComparison { get; set; }
        ...
        private void GetProductAnalysis()
        {
            using (var connection = GetConnection())
            {
                ...
                using (var command = new SqlCommand())
                {
                    ...
                    command.CommandText = @"select ProductName, Stock from Product where Stock <= 6 and IsDiscontinued = 0";
                    reader = command.ExecuteReader();
                    while (reader.Read())
                    {
        ...
        {
            if (GrossRevenueList.Count < 2)
            {
                RevenueGrowth = 0;
                return;
            }

            decimal previousTotal = GrossRevenueList.First().TotalAmount;
            decimal currentTotal = GrossRevenueList.Last().TotalAmount;

            RevenueGrowth = (currentTotal - previousTotal) / previousTotal;
        }
        ...
        private void CalculateRevenueComparison()
        {
            ...
            int periodLength = GrossRevenueList.Count / 2;
            var previousPeriod = GrossRevenueList.Take(periodLength).Sum(x => x.TotalAmount);
            var currentPeriod = GrossRevenueList.Skip(periodLength).Take(periodLength).Sum(x => x.TotalAmount);

            RevenueComparison = (currentPeriod - previousPeriod) / previousPeriod;
        }
